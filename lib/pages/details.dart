import 'package:flutter/material.dart';
import 'package:food_delivery/widget/widget_support.dart';

class Details extends StatefulWidget {
  const Details({super.key});

  @override
  State<Details> createState() => _DetailsState();
}

class _DetailsState extends State<Details> {
  int a = 1;

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      body: Padding(
        padding: const EdgeInsets.only(top: 50.0, left: 20.0, right: 20.0, bottom: 30.0),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            // Tombol kembali
            GestureDetector(
              onTap: () {
                Navigator.pop(context);
              },
              child: const Icon(
                Icons.arrow_back_ios_new_outlined,
                color: Colors.black,
              ),
            ),

            const SizedBox(height: 20),

            // Gambar makanan
            Image.asset(
              "images/salad2.png",
              width: MediaQuery.of(context).size.width,
              height: MediaQuery.of(context).size.height / 2.5,
              fit: BoxFit.fill,
            ),

            const SizedBox(height: 10.0),

            // Judul dan tombol tambah/kurang
            Row(
              children: [
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text("Mediterranean", style: AppWidget.semiBoldTextFieldStyle()),
                    Text("Chicken Salad", style: AppWidget.boldTextFieldStyle()),
                  ],
                ),
                const Spacer(),
                GestureDetector(
                  onTap: () {
                    if (a > 1) {
                      setState(() {
                        a--;
                      });
                    }
                  },
                  child: Container(
                    decoration: BoxDecoration(
                      color: Colors.black,
                      borderRadius: BorderRadius.circular(8),
                    ),
                    padding: const EdgeInsets.all(6),
                    child: const Icon(Icons.remove, color: Colors.white),
                  ),
                ),
                const SizedBox(width: 20.0),
                Text(a.toString(), style: AppWidget.semiBoldTextFieldStyle()),
                const SizedBox(width: 20.0),
                GestureDetector(
                  onTap: () {
                    setState(() {
                      a++;
                    });
                  },
                  child: Container(
                    decoration: BoxDecoration(
                      color: Colors.black,
                      borderRadius: BorderRadius.circular(8),
                    ),
                    padding: const EdgeInsets.all(6),
                    child: const Icon(Icons.add, color: Colors.white),
                  ),
                ),
              ],
            ),

            const SizedBox(height: 20.0),

            // Deskripsi
            Text(
              "Chicken Salad adalah hidangan segar berbahan dasar ayam yang dicampur dengan sayuran seperti selada, tomat, dan mentimun, biasanya disajikan dengan saus seperti mayones atau vinaigrette.",
              maxLines: 3,
              overflow: TextOverflow.ellipsis,
              style: AppWidget.lightTextFieldStyle(),
            ),

            const SizedBox(height: 20.0),

            // Waktu pengiriman
            Row(
              children: [
                Text("Delivery Time", style: AppWidget.semiBoldTextFieldStyle()),
                const SizedBox(width: 10),
                const Icon(Icons.alarm, color: Colors.black54),
                const SizedBox(width: 5),
                Text("30 min", style: AppWidget.semiBoldTextFieldStyle()),
              ],
            ),

            const Spacer(),

            // Harga & tombol Add to Cart
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text("Total Price", style: AppWidget.semiBoldTextFieldStyle()),
                    Text("\$${28 * a}", style: AppWidget.boldTextFieldStyle()),
                  ],
                ),
                Container(
                  decoration: BoxDecoration(
                    color: Colors.black,
                    borderRadius: BorderRadius.circular(12),
                  ),
                  padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10),
                  child: Row(
                    children: const [
                      Icon(Icons.shopping_cart_outlined, color: Colors.white),
                      SizedBox(width: 8),
                      Text("Add to cart", style: TextStyle(color: Colors.white)),
                    ],
                  ),
                )
              ],
            ),
          ],
        ),
      ),
    );
  }
}
